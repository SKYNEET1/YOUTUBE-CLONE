...
// imported a library - reactplayer : npm i react-player to play video.
...
function Playingvideo() {
  ...
  const [releatedvideo, setrelatedvideo] = useState();
  const { id } = useParams();
      ...
      <div className="w-full max-w-[1580px] flex flex-col lg:flex-row">
        <div className="flex flex-col lg:w-[calc(100%-350px)] xl:w-[100%-400px] px-4 py-3 lg:py-6">
          ...
          <div className="flex gap-x-6 font-semibold rounded-xl mt-4 text-xl">
            ...
            {/* in api inside video inside stats inside it we can found comments. */}
          </div>
        </div>
      </div>
      <div className="flex flex-col px-4 py-6 h-[calc(100vh-4.625rem)] overflow-y-scroll overflow-x-hidden lg:w-[350px] xl:w-[400px]">
     { 
     releatedvideo?.contents?.map((items,index)=>{
      if(items?.type!=="video") return null;
      return <Suggestedvideos key={index} video={items?.video}/>
      // passing as props if items is video then only we can render this component 
     })
      }
      </div>
    </div>
